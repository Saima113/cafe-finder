...
function getLocation() {
  ...
  if (!navigator.geolocation) {
    ...
    searchCitywide(28.6139, 77.209, "Delhi");
    return;
  }
...
async function searchCitywide(lat, lng, cityName) {
  ...
  if (allCafesFound.length > 0) {
    ...
    const uniqueCafes = Array.from(
      new Map(allCafesFound.map((c) => [c.id, c])).values()
    );

    // Separate into popular and hidden gems
    const popular = uniqueCafes.filter((c) => (c.userRatingCount || 0) > 100);
    const hiddenGems = uniqueCafes.filter(
      (c) => (c.userRatingCount || 0) <= 100 && (c.rating || 0) >= 4.0
    );
    ...
    console.log(
      `Found ${allCafes.length} cafes (${popular.length} popular, ${hiddenGems.length} hidden gems)`
    );
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      headers: {
        ...
        "X-Goog-FieldMask":
          "places.displayName,places.id,places.rating,places.photos,places.location,places.userRatingCount",
      },
...
function showMap() {
  if (!window.google || !window.google.maps) {
    alert(
      "Google Maps is not loaded. Please add the Maps JavaScript API script to your HTML."
    );
    return;
  }
